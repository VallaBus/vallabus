...
const statsCards = [
    {
        ...
        title: "Periodo",
        value: "31 días",
        subtitle: "1 al 31 de diciembre 2024"
    },
    {
        icon: 'file-bar-chart',
        title: "Registros",
...
frequencyBusIconsContainer.innerHTML = `
    <div class="w-full flex flex-wrap justify-center gap-1">
        ${Array(4).fill(`<i data-lucide="bus" class="h-8 w-8" style="color: ${COLORS.error}"></i>`).join('')}
        ${Array(1).fill(`<i data-lucide="bus" class="h-8 w-8" style="color: ${COLORS.successLight}"></i>`).join('')}
    </div>
    <div class="w-full flex flex-wrap justify-center gap-1">
...
fetch('data/puntualidad-evolucion.json')
    ...
    .then(data => {
        ...
        new Chart(monthlyTrendCtx, {
            type: 'line',
            data: {
                labels: data.map(item => item.Fecha.replace('., 2024', '')),
                datasets: [{
